# VassilFlow Maintainer Orchestrator — design notes

This document explains the *thinking* behind the `vassilflow-maintainer-orchestrator` skill: what it is for, the boundaries that make it safe to run, and the principles that shape how it reviews. It is written for VassilFlow maintainers who run the skill, and for anyone in the community who wants to understand — or adapt — the pattern of delegating issue and PR triage to an agent.
...
> `.agent/skills/vassilflow-maintainer-orchestrator/SKILL.md`
