...
**Goal:** Add optional Langfuse observability support to VassilFlow while preserving existing LangSmith tracing and allowing both providers to be enabled at the same time.
...
Run: `cd backend && uv run ruff check packages/harness/vassilflow/config/tracing_config.py packages/harness/vassilflow/models/factory.py packages/harness/vassilflow/tracing`
