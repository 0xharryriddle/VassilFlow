#!/usr/bin/env python3
"""Create a redacted VassilFlow support bundle for troubleshooting."""
...
try:
    import yaml
except Exception:  # pragma: no cover - only exercised in broken environments
    yaml = None
...
def _candidate_thread_data_dirs(project_root: Path, thread_id: str) -> list[Path]:
    ...
    candidates = [
        project_root / ".vassilflow" / "threads" / thread_id / "user-data",
        project_root / "backend" / ".vassilflow" / "threads" / thread_id / "user-data",
    ]
    for base in (project_root / ".vassilflow" / "users", project_root / "backend" / ".vassilflow" / "users"):
        if base.exists():
...
def build_triage_report(
    *,
    ...
    """Build the stable machine-readable summary maintainers should read first."""
...
def render_issue_summary(triage: dict[str, Any]) -> str:
    """Render Markdown that users can paste into a GitHub issue."""
    git = triage["git"]
    doctor = triage["doctor"]
    versions = triage["versions"]
    lines = [
        "## VassilFlow support bundle summary",
        "",
...
def render_ai_issue_draft(triage: dict[str, Any], issue_summary: str, doctor: dict[str, Any] | None) -> str:
    ...
    lines = [
        "# AI issue draft",
        "",
        "Use this when a coding agent or AI assistant files a VassilFlow bug report.",
        ...
        "```",
        "",
        "### How are you running VassilFlow?",
        "",
...
def _default_out_path(project_root: Path) -> Path:
    ...
    return project_root / ".vassilflow" / "support-bundles" / f"vassilflow-support-bundle-{timestamp}.zip"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--project-root", type=Path, default=repo_root, help="VassilFlow project root")
