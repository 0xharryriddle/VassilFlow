...
if [ -f "$ENV_FILE" ]; then
    COMPOSE_CMD=(docker compose --env-file "$ENV_FILE" -p "$COMPOSE_PROJECT_NAME" -f "$DOCKER_DIR/docker-compose.yaml")
else
...
detect_sandbox_mode() {
    ...
    if [[ "$sandbox_use" == *"vassilflow.community.aio_sandbox:AioSandboxProvider"* ]]; then
