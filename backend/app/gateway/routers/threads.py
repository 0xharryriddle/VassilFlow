...
:func:`vassilflow.runtime.serialization.serialize_channel_values` to
...
from vassilflow.runtime.context_compaction import (
...
async def _branch_targets_latest_turn(checkpointer: Any, thread_id: str, target_message_ids: set[str]) -> bool:
    """Return True when the target turn is the final visible turn in current state."""
    config = {"configurable": {"thread_id": thread_id, "checkpoint_ns": ""}}
    try:
        async for checkpoint_tuple in checkpointer.alist(config, limit=_BRANCH_HISTORY_SCAN_LIMIT):
...
class ThreadSearchRequest(BaseModel):
    ...
    def _validate_metadata_filters(cls, v: dict[str, Any]) -> dict[str, Any]:
        ...
        See ``vassilflow.persistence.json_compat`` for the shared validators.
        """
        if not v:
            return v
        from vassilflow.persistence.json_compat import validate_metadata_filter_key, validate_metadata_filter_value
...
async def delete_thread_data(thread_id: str, request: Request) -> ThreadDeleteResponse:
    ...
    Cleans VassilFlow-managed thread directories, removes checkpoint data,
...
async def get_thread_history(thread_id: str, body: ThreadHistoryRequest, request: Request) -> list[HistoryEntry]:
    ...
    :func:`~vassilflow.runtime.serialization.serialize_channel_values`.
