...
These endpoints aggregate run history, token usage, and estimated cost for the
current user. They require a SQL persistence backend because memory mode does
not keep historical run rows after process exit.
"""
...
class ConsoleStatsResponse(BaseModel):
    ...
    total_cost: float | None = Field(default=None, description="Estimated spend across priced runs")
    currency: str | None = Field(default=None, description="Display currency from model pricing config")


class ConsoleRunItem(BaseModel):
    """One run in the cross-thread listing."""
    ...
    duration_seconds: float | None = Field(default=None, description="Wall-clock duration, or live elapsed time for active runs")
    total_tokens: int = 0
    message_count: int = 0
    cost: float | None = Field(default=None, description="Estimated spend for this run")
...
class ConsoleUsageDay(BaseModel):
    ...
    date: str = Field(..., description="Local date (YYYY-MM-DD) per the requested timezone offset")
    total_tokens: int = 0
    input_tokens: int = 0
    output_tokens: int = 0
    runs: int = 0
...
class ConsoleUsageModelBreakdown(BaseModel):
    ...
    tokens: int = 0
    runs: int = Field(default=0, description="Runs that used this model")
    cost: float | None = Field(default=None, description="Estimated spend for this model")
...
class ConsoleUsageResponse(BaseModel):
    """Daily token usage plus per-model breakdown."""

    days: list[ConsoleUsageDay]
    by_model: dict[str, ConsoleUsageModelBreakdown]
    total_tokens: int
    total_runs: int
    total_cost: float | None = Field(default=None, description="Estimated spend for the requested window")
    currency: str | None = Field(default=None, description="Display currency from model pricing config")
...
def _as_utc(dt: datetime | None) -> datetime | None:
    """Normalize DB timestamps; SQLite round-trips them naive."""
    if dt is None:
        return None
...
def _build_pricing_map() -> dict[str, _ModelPricing]:
    """Collect prices from optional ``models[*].pricing`` config metadata."""
    try:
        models = get_app_config().models
    except Exception:  # pragma: no cover - cost display must not break console
    ...
    for model_cfg in models or []:
        ...
        entry = _ModelPricing(
            input_price,
            output_price,
            str(raw.get("currency") or "USD").upper(),
            cache_hit_price,
        )
...
def _token_cost(input_tokens: int, output_tokens: int, price: _ModelPricing, cache_read_tokens: int = 0) -> float:
    """Estimate cost with cache-hit input tokens billed at the configured hit price."""
    input_total = max(int(input_tokens or 0), 0)
    cache_read = min(max(int(cache_read_tokens or 0), 0), input_total)
    uncached = input_total - cache_read
    ...
    return (uncached / 1_000_000) * price.input_per_million + (cache_read / 1_000_000) * hit_price + (int(output_tokens or 0) / 1_000_000) * price.output_per_million
...
def _run_cost(
    ...
    *,
    ...
    """Estimate one run's spend, preferring the per-model breakdown."""
    cost = 0.0
    priced = False
...
@router.get(
    ...
    description="Headline counters scoped to the current user.",
)
@require_permission("runs", "read")
async def console_stats(request: Request) -> ConsoleStatsResponse:
    """Return dashboard headline counters."""
    ...
    except Exception:  # pragma: no cover - stats must not 500 on a bad agent dir
...
@router.get(
    ...
    description="Cross-thread run history for the current user, newest first.",
)
@require_permission("runs", "read")
async def console_runs(
    ...
    """Return a page of runs across all threads."""
    sf = _session_factory_or_503()
    user_id = await get_current_user(request)
...
@router.get(
    ...
    description="Daily token usage series plus per-model breakdown.",
)
@require_permission("runs", "read")
async def console_usage(
    ...
    for i in range(days):
        date_key = (start_local + timedelta(days=i)).isoformat()
        day_buckets[date_key] = ConsoleUsageDay(date=date_key)
