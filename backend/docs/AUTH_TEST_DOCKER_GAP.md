...
| TC-DOCKER-05 (credential surfacing) | `reset_admin` writes `.vassilflow/admin_initial_credentials.txt` with mode 0600 and logs only the path; legacy `.vassilflow/admin_initial_credentials.txt` is still possible when using a legacy runtime home. The only Docker-unique step is whether the bind mount projects this path onto the host, which is a `docker compose` config check, not a runtime behavior change |
