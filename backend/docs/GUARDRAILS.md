...
> **Context:** [Issue #1213](upstream issue #1213) — VassilFlow keeps Docker sandboxing and human approval via `ask_clarification`, but still needs a deterministic, policy-driven authorization layer for tool calls. An agent running autonomous multi-step tasks can execute any loaded tool with any arguments. Guardrails add a middleware that evaluates every tool call against a policy **before** execution.
...
The simplest option. Ships with VassilFlow. Block or allow tools by name. No external packages, no passport, no network.
...
This creates:
- `~/.aport/vassilflow/config.yaml` -- evaluator config (local or API mode)
- `~/.aport/vassilflow/aport/passport.json` -- OAP passport with capabilities and limits
...
Any provider that accepts `framework` as a kwarg and implements `evaluate`/`aevaluate` works. The OAP standard defines the passport format and decision codes; VassilFlow doesn't care which provider reads them.
...
A custom OAP provider can implement any evaluation strategy -- the VassilFlow middleware doesn't care how the provider reaches its decision.

**Try it:**
1. Install and set up as above
2. Start VassilFlow and ask: "Create a file called test.txt with content hello"
...
2. Add the config
3. Start VassilFlow and ask: "Use bash to delete test.txt"
...
| Field | Example use |
|---|---|
| `user_id` | Attach the authenticated VassilFlow user to a provider-side policy or audit record |
...
class ContextAwareGuardrailProvider:
    ...
    async def aevaluate(self, request):
        ...
        # (VassilFlow enforces a blocking-IO gate in CI). If your policy
    ...
    def _decide(self, request):
        # 1. Normalize VassilFlow request data into policy context.
        context = {
        ...
        # 3. Convert the policy result back to VassilFlow's decision object.
        return GuardrailDecision(
            allow=result["allow"],
            reasons=[
                GuardrailReason(
...
VassilFlow loads providers via `resolve_variable()` -- the same mechanism used for models, tools, and sandbox providers. The `use:` field is a Python class path: `package.module:ClassName`.

The provider is instantiated with `**config` kwargs if `config:` is set, plus `framework="vassilflow"` is always injected. Accept `**kwargs` to stay forward-compatible:

```python
class YourProvider:
    def __init__(self, framework: str = "generic", **kwargs):
        # framework="vassilflow" tells you which config dir to use
        ...
```
