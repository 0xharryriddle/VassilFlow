...
VassilFlow supports single sign-on (SSO) via any OpenID Connect (OIDC) 2.0 compliant provider. This includes Keycloak, Google Workspace, Azure AD, Okta, and many others.
...
- **Existing VassilFlow JWT** — After successful OIDC authentication, VassilFlow creates its own JWT session cookie. The OIDC provider's tokens are never exposed to the browser.
...
providers:
  <provider-id>:
    ...
    # User provisioning
    auto_create_users: true         # Auto-create VassilFlow account on first SSO login (default: true)
...
3. Create a new realm called `vassilflow`
4. In the `vassilflow` realm, go to **Clients** → **Create client**
5. Configure:
   - **Client ID**: `vassilflow`
...
1. In the `vassilflow` realm, go to **Users** → **Add user**
...
5. After successful authentication, you'll be redirected back to the VassilFlow workspace
...
- Password changes must be done through the SSO provider, not VassilFlow
