...
compare candidates with the same VassilFlow workload.
...
python scripts/sandbox_memory_profile.py \
  --namespace vassilflow \
  --selector app=vassilflow-sandbox \
  --sample empty \
  --include-processes \
  --format markdown
```
...
same VassilFlow workload has been measured on both the current AIO sandbox and the
candidate backend.

For an experimental provider PR, prefer `Related to #3213` unless the PR also
includes reproducible VassilFlow workload data that demonstrates the target memory
