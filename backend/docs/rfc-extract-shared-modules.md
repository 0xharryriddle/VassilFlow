...
Gateway (`app/gateway/routers/skills.py`, `uploads.py`) and Client (`vassilflow.client`, legacy implementation `deerflow/client.py`) each independently implement the same business logic:
...
app.gateway.routers.uploads ──┤── calls ──→  vassilflow.skills.installer
vassilflow.client             ──┘              vassilflow.uploads.manager
```

- Shared modules live in the harness layer (`vassilflow.*`), pure business logic, no FastAPI dependency
...
### 3.1 `vassilflow.skills.installer`
...
### 3.2 `vassilflow.uploads.manager`
...
**`vassilflow.client` / legacy `deerflow/client.py`**:
...
| Alternative | Why Not |
|-------------|---------|
| Keep logic in Gateway, Client calls Gateway via HTTP | Adds network dependency to embedded Client; defeats the purpose of `VassilFlowClient` as an in-process API |
...
**None.** All public APIs (Gateway HTTP endpoints, `VassilFlowClient` methods) retain their existing signatures and return formats. The `SkillAlreadyExistsError` is a subclass of `ValueError`, so existing `except ValueError` handlers still catch it.
