...
VassilFlow supports configurable MCP servers and skills to extend its capabilities, which are loaded from a dedicated `extensions_config.json` file in the project root directory.
...
`tool_call_timeout` only applies to `stdio` servers. `http` and `sse` servers
use transport-level timeouts, and VassilFlow logs a warning if
`tool_call_timeout` is configured for those transports.

## Filesystem MCP Servers

VassilFlow already provides built-in file tools for thread-scoped workspace access.
Do not add an MCP filesystem server for the same VassilFlow workspace. The
overlapping file tools use different path semantics, which can make LLM tool
selection and file access behavior unstable.

VassilFlow does not currently adapt the MCP Roots mode for filesystem servers. In
particular, it does not publish per-thread MCP roots or map VassilFlow sandbox
paths such as `/mnt/user-data/...` to paths accepted by
`@modelcontextprotocol/server-filesystem`. Use VassilFlow's built-in file tools
for VassilFlow workspace files.

## OAuth Support (HTTP/SSE MCP Servers)

For `http` and `sse` MCP servers, VassilFlow supports OAuth token acquisition and automatic token refresh.
...
MCP servers expose tools that are automatically discovered and integrated into VassilFlow's agent system at runtime. Once enabled, these tools become available to agents without additional code changes.
