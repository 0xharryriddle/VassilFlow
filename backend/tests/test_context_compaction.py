...
from vassilflow.runtime import context_compaction
from vassilflow.runtime.context_compaction import compact_thread_context
...
class _FakeCompactionMiddleware:
    ...
    async def acompact_state(self, state, runtime, *, force=False):
        ...
        summary_message = HumanMessage(content="Here is a summary of the conversation to date:\n\nCOMPRESSED SUMMARY", name="summary")
        return SimpleNamespace(
...
@pytest.mark.asyncio
async def test_compact_thread_context_writes_summary_message_and_bumps_messages_channel(monkeypatch):
    messages = [
    ...
    checkpointer = _FakeCheckpointer(
        {
            "id": "ckpt-old",
            "channel_values": {"messages": messages, "sandbox": _RejectDeepcopy()},
            "channel_versions": {"messages": 7, "title": 2},
        }
    )
    ...
    assert written_messages[0].name == "summary"
    assert written_messages[1] == messages[-1]
    ...
    assert new_versions == {"messages": 8}
