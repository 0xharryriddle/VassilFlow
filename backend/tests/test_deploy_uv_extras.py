...
def _shell_works(path: str | Path) -> bool:
    try:
...
def _find_sh() -> str:
    ...
    if resolved and _shell_works(resolved):
        return resolved
    for candidate in (
        Path(r"C:\Program Files\Git\usr\bin\sh.exe"),
        Path(r"C:\Program Files\Git\bin\sh.exe"),
    ):
        if candidate.is_file() and _shell_works(candidate):
            return str(candidate)
    pytest.skip("POSIX sh is required for deploy extras tests")
...
def _find_bash() -> str:
    ...
    for candidate in (
        ...
        if candidate.is_file() and _shell_works(candidate):
            return str(candidate)
    pytest.skip("bash is required for deploy.sh tests")
...
def test_backend_dockerfile_expands_multiple_uv_extras(tmp_path):
    ...
    env["PATH"] = f"{bin_dir}{os.pathsep}{Path(sh).parent}{os.pathsep}{env.get('PATH', '')}"
...
def test_deploy_uses_dotenv_without_sourcing_shell_syntax(tmp_path):
    ...
    (worktree / ".env").write_text(
        f"DATABASE_URL=postgresql://user:pass@localhost/db?sslmode=require&application_name=vassilflow\nUNSAFE=$(touch {shlex.quote(str(marker))})\nUV_EXTRAS=discord\n",
        encoding="utf-8",
    )
    ...
    assert any(arg.replace("\\", "/").endswith("/repo/.env") for arg in args)
