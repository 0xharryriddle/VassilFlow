...
def reset_api_key_warned():
    ...
    import vassilflow.community.serper.tools as serper_mod
...
def mock_config_with_key():
    with patch("vassilflow.community.serper.tools.get_app_config") as mock:
...
class TestGetApiKey:
    def test_returns_config_key_when_present(self):
        with patch("vassilflow.community.serper.tools.get_app_config") as mock:
            ...
            from vassilflow.community.serper.tools import _get_api_key
...
class TestCoerceMaxResults:
    def test_returns_value_when_valid_positive_int(self):
        from vassilflow.community.serper.tools import _coerce_max_results
...
class TestSafePublicUrl:
    def test_https_public_hostname_passes(self):
        from vassilflow.community.serper.tools import _safe_public_url
...
class TestWebSearchTool:
    def test_basic_search_returns_normalized_results(self, mock_config_with_key):
        ...
        with patch("vassilflow.community.serper.tools.httpx.Client") as mock_client_cls:
...
def test_package_exports_image_search_tool():
    from vassilflow.community.serper import image_search_tool
    from vassilflow.community.serper.tools import image_search_tool as direct_image_search_tool
