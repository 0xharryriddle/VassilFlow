...
def test_redact_data_masks_url_credentials_and_cli_flag_secrets():
    data = {
        ...
        "database_url": "postgres://vfuser:vfpass@db:5432/vassilflow",
        "mcpServers": {
            "svc": {
                "command": "npx",
    ...
    assert "vfpass" not in redacted["database_url"]
    assert redacted["database_url"] == "postgres://<redacted>@db:5432/vassilflow"
...
def test_redact_text_masks_url_userinfo_and_query_secrets():
    text = "\n".join(
        [
            ...
            "postgres://vfuser:vfpass@db:5432/vassilflow",
...
def test_redact_text_masks_home_directory_paths():
    text = "\n".join(
        [
            "/Users/alice/VassilFlow/config.yaml",
            "/home/bob/VassilFlow/config.yaml",
            r"C:\Users\carol\VassilFlow\config.yaml",
        ]
    )
    ...
    assert "/Users/<user>/VassilFlow/config.yaml" in redacted
    assert "/home/<user>/VassilFlow/config.yaml" in redacted
    assert r"C:\Users\<user>\VassilFlow\config.yaml" in redacted
...
def test_create_support_bundle_masks_hardcoded_env_secret(tmp_path):
    ...
    with zipfile.ZipFile(output_path) as zf:
        all_text = "\n".join(_zip_text(output_path, name) for name in zf.namelist())
...
tools:
  - name: web_search
    use: vassilflow.community.brave.tools:web_search_tool
...
def test_create_support_bundle_writes_ai_triage_entrypoints(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "collect_git_summary",
        lambda _project_root: {
            "branch": {"ok": True, "stdout": "feat/support-bundle", "stderr": ""},
            "head": {"ok": True, "stdout": "abc123", "stderr": ""},
            "upstream": {"ok": True, "stdout": "origin/main", "stderr": ""},
            "status_short": {
                "ok": True,
                "stdout": "## feat/support-bundle...origin/main\n M README.md",
                "stderr": "",
            },
    ...
    assert "VassilFlow support bundle summary" in issue_summary
    ...
    assert "VassilFlow bug report" in issue_draft
    ...
    assert "VassilFlow Health Check" in issue_draft
    assert "### How are you running VassilFlow?" in issue_draft
    ...
    assert "branch: feat/support-bundle" in issue_draft
...
def test_thread_summary_lists_files_without_file_contents(tmp_path):
    project_root = tmp_path / "project"
    outputs = project_root / ".vassilflow" / "threads" / "thread-123" / "user-data" / "outputs"
    uploads = project_root / ".vassilflow" / "threads" / "thread-123" / "user-data" / "uploads"
