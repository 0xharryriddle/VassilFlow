...
from vassilflow.agents.middlewares import title_middleware as title_middleware_module
from vassilflow.agents.middlewares.dynamic_context_middleware import _DYNAMIC_CONTEXT_REMINDER_KEY
from vassilflow.agents.middlewares.title_middleware import TitleMiddleware
from vassilflow.config.title_config import TitleConfig, get_title_config, set_title_config
...
class TestTitleMiddlewareCoreLogic:
    ...
    def test_async_local_fallback_does_not_format_unused_prompt_template(self, monkeypatch):
        ...
        state = {
            "messages": [
                HumanMessage(content="title fallback should be local"),
                AIMessage(content="sure"),
            ]
        }
        result = asyncio.run(middleware._agenerate_title_result(state))

        assert result == {"title": "title fallback shoul..."}
        create_chat_model.assert_not_called()
    ...
    def test_async_title_model_falls_back_when_prompt_template_is_invalid(self, monkeypatch):
        ...
        state = {
            "messages": [
                HumanMessage(content="write tests for title"),
                AIMessage(content="sure"),
            ]
        }
        result = asyncio.run(middleware._agenerate_title_result(state))

        assert result == {"title": "write tests for titl..."}
        create_chat_model.assert_not_called()
    ...
    def test_should_generate_title_partial_exchange_allows_user_only(self):
        ...
        _set_test_title_config(enabled=True)
        middleware = TitleMiddleware()
        state = {"messages": [HumanMessage(content="Only user message")]}
    ...
    def test_should_generate_title_partial_exchange_skips_when_titled(self):
        _set_test_title_config(enabled=True)
        middleware = TitleMiddleware()
        state = {"messages": [HumanMessage(content="Question")], "title": "Already set"}
    ...
    def test_sync_generate_title_from_dict_messages(self):
        ...
        assert middleware._generate_title_result(state) == {"title": "write tests"}
    ...
    def test_partial_exchange_with_dict_human_message(self):
        ...
        middleware = TitleMiddleware()
        state = {"messages": [{"role": "user", "content": "write tests"}]}
    ...
    def test_partial_exchange_ignores_dict_dynamic_context_reminder(self):
        ...
        state = {
            "messages": [
                {
                    "type": "human",
                    "content": "<memory>User prefers short titles.</memory>",
                    "additional_kwargs": {"hide_from_ui": True, _DYNAMIC_CONTEXT_REMINDER_KEY: True},
                },
                {"type": "human", "content": "write tests", "additional_kwargs": {}},
            ]
        }

        assert middleware._should_generate_title(state, allow_partial_exchange=True) is True
        assert middleware._generate_title_result(state, allow_partial_exchange=True) == {"title": "write tests"}
