...
from vassilflow.config.paths import Paths, join_host_path
...
def test_remote_backend_create_forwards_effective_user_id(monkeypatch):
    ...
    remote_mod = importlib.import_module("vassilflow.community.aio_sandbox.remote_backend")
...
def test_acquire_drops_dead_cached_sandbox(tmp_path, monkeypatch):
    ...
    provider._backend.create = MagicMock(
        return_value=aio_mod.SandboxInfo(
            ...
            container_name="vassilflow-sandbox-sandbox-dead",
        )
    )
    ...
    provider._warm_pool = {
        "sandbox-warm-dead": (
            aio_mod.SandboxInfo(
                ...
                container_name="vassilflow-sandbox-sandbox-warm-dead",
            ),
            0.0,
        )
    }
