...
def test_smoke_test_scripts_detect_vassilflow_names():
    ...
    assert 'grep -Eiq "vassilflow"' in health_check
...
def test_blocking_io_guard_skill_uses_vassilflow_paths_and_scope():
    ...
    assert "`vassilflow.*` implementation imports" in reference
...
def test_find_skills_installer_prefers_vassilflow_workspace_marker():
    ...
    assert "vassilflow.code-workspace not found" in install_script
