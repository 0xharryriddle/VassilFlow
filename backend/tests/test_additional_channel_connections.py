...
async def _seed_state(repo, provider: str, state: str, owner_user_id: str = "vassilflow-user-1") -> None:
...
def test_feishu_connect_command_binds_identity(tmp_path):
    ...
    async def go():
        ...
        connections = await repo.list_connections("vassilflow-user-1")
...
def test_additional_channels_attach_owner_identity(tmp_path):
    ...
    async def go():
        ...
        for channel, inbound in cases:
            ...
            assert attached.owner_user_id == "vassilflow-user-1"
