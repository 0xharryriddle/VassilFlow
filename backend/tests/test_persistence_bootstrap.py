...
async def test_legacy_missing_channel_tables_get_backfilled(tmp_path: Path) -> None:
    ...
    try:
        ...
        # branch (has_application_tables=True, no alembic_version) while the
...
class TestDecideState:
    def test_empty(self):
        assert _decide_state({"has_alembic_version": False, "has_application_tables": False}) == "empty"

    def test_empty_with_unrelated_tables(self):
        # LangGraph checkpointer tables present but VassilFlow has nothing yet.
        # ``has_application_tables`` is derived from the metadata intersection in
        # production, so the only thing the decision function needs is the
        # bool itself.
        assert _decide_state({"has_alembic_version": False, "has_application_tables": False}) == "empty"

    def test_legacy(self):
        assert _decide_state({"has_alembic_version": False, "has_application_tables": True}) == "legacy"

    def test_versioned(self):
        assert _decide_state({"has_alembic_version": True, "has_application_tables": True}) == "versioned"
    ...
    def test_versioned_takes_precedence_over_empty(self):
        ...
        assert _decide_state({"has_alembic_version": True, "has_application_tables": False}) == "versioned"
