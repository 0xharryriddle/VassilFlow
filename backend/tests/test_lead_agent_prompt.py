...
from vassilflow.config.subagents_config import CustomSubagentConfig, SubagentsAppConfig
...
def test_apply_prompt_template_uses_vassilflow_default_identity(monkeypatch):
    config = SimpleNamespace(
        sandbox=SimpleNamespace(mounts=[]),
        skills=SimpleNamespace(container_path="/mnt/skills"),
    )
...
def test_get_memory_context_uses_explicit_app_config_without_global_config(monkeypatch):
    ...
    monkeypatch.setattr("vassilflow.agents.memory.get_memory_data", fake_get_memory_data)
    monkeypatch.setattr("vassilflow.agents.memory.format_memory_for_injection", fake_format_memory_for_injection)
