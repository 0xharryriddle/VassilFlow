...
def test_queue_add_stores_user_id():
    q = MemoryUpdateQueue()
    with patch("vassilflow.agents.memory.queue.get_memory_config", return_value=MemoryConfig(enabled=True)), patch.object(q, "_reset_timer"):
...
def test_queue_process_passes_user_id_to_updater():
    ...
    with patch("vassilflow.agents.memory.updater.MemoryUpdater", return_value=mock_updater):
        q._process_queue()
...
def test_add_nowait_keeps_different_users_separate():
    ...
    with (
        patch("vassilflow.agents.memory.queue.get_memory_config", return_value=MemoryConfig(enabled=True)),
