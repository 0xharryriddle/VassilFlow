...
def test_default_target_accepts_legacy_home(tmp_path, monkeypatch):
    legacy_home = tmp_path / "legacy-home"
    monkeypatch.delenv("VASSILFLOW_HOME", raising=False)
    monkeypatch.setenv("DEER_FLOW_HOME", str(legacy_home))

    assert load_memory_sample.default_target(tmp_path) == legacy_home / "memory.json"
...
def test_default_target_falls_back_to_existing_legacy_runtime_dir(tmp_path, monkeypatch):
    monkeypatch.delenv("VASSILFLOW_HOME", raising=False)
    monkeypatch.delenv("DEER_FLOW_HOME", raising=False)
    legacy_home = tmp_path / "backend" / ".deer-flow"
    legacy_home.mkdir(parents=True)
