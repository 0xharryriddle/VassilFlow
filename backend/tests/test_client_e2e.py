"""End-to-end tests for VassilFlowClient.
...
Only VASSILFLOW_HOME is redirected to tmp_path for filesystem isolation.
...
def e2e_env(tmp_path, monkeypatch):
    ...
    - VASSILFLOW_HOME → tmp_path (all thread data lands in a temp dir)
    - VASSILFLOW_PROJECT_ROOT → repository root (shared skills/config assets
...
@pytest.fixture()
def client(e2e_env):
    """A VassilFlowClient wired to the isolated e2e_env."""
    return VassilFlowClient(checkpointer=None, thinking_enabled=False)
...
class TestLifecycleAndConfig:
    ...
    def test_plan_mode_config_key(self, e2e_env):
        ...
        c = VassilFlowClient(checkpointer=None, plan_mode=False)
        ...
        c2 = VassilFlowClient(checkpointer=None, plan_mode=True)
