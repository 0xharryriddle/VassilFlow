...
def test_get_skills_prompt_section_includes_self_evolution_rules(monkeypatch):
    ...
    monkeypatch.setattr("vassilflow.agents.lead_agent.prompt._get_enabled_skills", lambda: skills)
    monkeypatch.setattr(
        "vassilflow.config.get_app_config",
...
def test_get_skills_prompt_section_includes_self_evolution_rules_without_skills(monkeypatch):
    monkeypatch.setattr("vassilflow.agents.lead_agent.prompt._get_enabled_skills", lambda: [])
    monkeypatch.setattr(
        "vassilflow.config.get_app_config",
        lambda: SimpleNamespace(
            skills=SimpleNamespace(container_path="/mnt/skills"),
...
def test_get_skills_prompt_section_uses_explicit_config_for_enabled_skills(monkeypatch):
    ...
    monkeypatch.setattr("vassilflow.agents.lead_agent.prompt._get_enabled_skills", lambda: [_make_skill("global-skill")])
    monkeypatch.setattr("vassilflow.config.get_app_config", fail_get_app_config)
    monkeypatch.setattr(
        "vassilflow.agents.lead_agent.prompt.get_or_new_skill_storage",
...
def test_make_lead_agent_empty_skills_passed_correctly(monkeypatch):
    ...
    monkeypatch.setattr("vassilflow.tools.get_available_tools", lambda **kwargs: [])
...
def test_make_lead_agent_all_legacy_skills_preserve_all_tools(monkeypatch):
    ...
    monkeypatch.setattr("vassilflow.tools.get_available_tools", lambda **kwargs: [NamedTool("bash"), NamedTool("read_file")])
