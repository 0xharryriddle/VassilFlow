...
task_tool_module = importlib.import_module("vassilflow.tools.builtins.task_tool")
...
def test_task_tool_threads_runtime_app_config_to_subagent_dependencies(monkeypatch):
    ...
    monkeypatch.setattr("vassilflow.tools.get_available_tools", fake_get_available_tools)
...
def test_task_tool_emits_running_and_completed_events(monkeypatch):
    ...
    monkeypatch.setattr(task_tool_module, "get_stream_writer", lambda: events.append)
    monkeypatch.setattr(task_tool_module.asyncio, "sleep", _no_sleep)
    # task_tool lazily reaches the implementation tools module at call time, so patch that module-level function.
    monkeypatch.setattr("vassilflow.tools.get_available_tools", get_available_tools)
