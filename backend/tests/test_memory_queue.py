...
def test_add_nowait_cancels_existing_timer_and_starts_immediate_timer() -> None:
    ...
    with (
        patch("vassilflow.agents.memory.queue.get_memory_config", return_value=_memory_config(enabled=True)),
        patch("vassilflow.agents.memory.queue.threading.Timer", return_value=created_timer) as timer_cls,
    ):
...
def test_process_queue_updates_different_agents_in_same_thread_separately() -> None:
    ...
    with (
        patch("vassilflow.agents.memory.updater.MemoryUpdater", return_value=mock_updater),
        patch("vassilflow.agents.memory.queue.time.sleep"),
    ):
        queue.flush()
