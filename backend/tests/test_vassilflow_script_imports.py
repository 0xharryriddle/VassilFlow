...
def test_tool_error_degradation_detector_uses_vassilflow_package_imports():
    ...
    assert "from vassilflow.agents.middlewares.thread_data_middleware import ThreadDataMiddleware" in content
    assert "from vassilflow.agents.middlewares.tool_error_handling_middleware import build_subagent_runtime_middlewares" in content
