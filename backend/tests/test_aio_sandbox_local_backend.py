...
def test_format_container_mount_uses_mount_syntax_for_docker_windows_paths():
    args = _format_container_mount("docker", "D:/VassilFlow/backend/.vassilflow/threads", "/mnt/threads", False)

    assert args == [
        "--mount",
        "type=bind,src=D:/VassilFlow/backend/.vassilflow/threads,dst=/mnt/threads",
    ]
...
def test_start_container_logs_redacted_env_values(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="vassilflow.community.aio_sandbox.local_backend"):
...
def test_resolve_docker_bind_host_uses_ipv6_loopback_for_ipv6_sandbox_host(monkeypatch):
    ...
    monkeypatch.delenv("DEER_FLOW_SANDBOX_HOST", raising=False)
    monkeypatch.delenv("VASSILFLOW_SANDBOX_BIND_HOST", raising=False)
    monkeypatch.setenv("VASSILFLOW_SANDBOX_HOST", "[::1]")

    assert _resolve_docker_bind_host() == "[::1]"


def test_resolve_docker_bind_host_logs_selected_bind_reason(caplog):
    with caplog.at_level(logging.DEBUG, logger="vassilflow.community.aio_sandbox.local_backend"):
...
def test_resolve_docker_bind_host_allows_explicit_override(monkeypatch):
    ...
    monkeypatch.setenv("VASSILFLOW_SANDBOX_BIND_HOST", "192.0.2.10")
...
def test_start_container_keeps_apple_container_port_format(monkeypatch):
    ...
    monkeypatch.delenv("DEER_FLOW_SANDBOX_BIND_HOST", raising=False)
    monkeypatch.setenv("VASSILFLOW_SANDBOX_BIND_HOST", "127.0.0.1")
    monkeypatch.delenv("VASSILFLOW_SANDBOX_HOST", raising=False)
