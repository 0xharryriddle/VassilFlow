...
@pytest.fixture
def isolated_vassilflow_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setenv("VASSILFLOW_EXTENSIONS_CONFIG_PATH", str(staged_extensions_config))
...
def _reset_process_singletons(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ``config.yaml``/``extensions_config.json`` and ``VASSILFLOW_HOME``, so the
    caches below must be reset before app creation:

    - app_config / extensions_config: parsed config file caches.
    - paths: ``VASSILFLOW_HOME``-derived filesystem paths.
...
def test_stream_run_executes_real_lead_agent_setup_agent_business_path(isolated_app, isolated_vassilflow_home: Path):
    ...
    with (
        ...
        expected_soul = isolated_vassilflow_home / "users" / auth_user_id / "agents" / agent_name / "SOUL.md"
        assert expected_soul.exists(), f"setup_agent did not write SOUL.md. tmp tree: {sorted(str(p.relative_to(isolated_vassilflow_home)) for p in isolated_vassilflow_home.rglob('SOUL.md'))}"
        assert f"Agent name: {agent_name}" in expected_soul.read_text(encoding="utf-8")
        assert not (isolated_vassilflow_home / "users" / "default" / "agents" / agent_name).exists()
