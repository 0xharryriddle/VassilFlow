...
from vassilflow.workspace_changes import (
...
from vassilflow.workspace_changes.api import get_workspace_changes_response
from vassilflow.workspace_changes.scanner import SAMPLE_BYTES, is_sensitive_workspace_path


def _roots(tmp_path: Path) -> list[WorkspaceRoot]:
...
def test_compare_snapshots_treats_utf16_markdown_as_text(tmp_path):
    ...
    before = scan_workspace_roots(roots)
    (workspace / "guide.md").write_bytes("# Title\n\nhello\n".encode("utf-16"))
    after = scan_workspace_roots(roots)
...
def test_compare_snapshots_treats_utf8_markdown_crossing_sample_boundary_as_text(
    ...
    before = scan_workspace_roots(roots)
    content = ("a" * (SAMPLE_BYTES - 1)) + "VassilFlow\nrest\n"
    ...
    assert change.path == "/mnt/user-data/workspace/guide.md"
    assert change.binary is False
    assert "VassilFlow" in change.diff
...
def test_count_diff_lines_ignores_only_real_headers():
    from vassilflow.workspace_changes.diff import _count_diff_lines
...
async def test_workspace_changes_route_forwards_include_files_flag():
    ...
    class FakeRequest:
        app = FakeApp()
        _vassilflow_test_bypass_auth = True
