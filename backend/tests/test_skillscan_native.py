...
from vassilflow.skills.skillscan import StaticScanBlockedError, enforce_static_scan, scan_archive_preflight, scan_skill_dir
...
def test_archive_member_count_cap_blocks(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from vassilflow.skills.skillscan import orchestrator
...
async def test_llm_scanner_receives_static_findings_context(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("vassilflow.skills.security_scanner.create_chat_model", lambda **kwargs: FakeModel())
