...
class TestProviders:
    ...
    def test_llm_providers_cover_config_example_families(self):
        ...
        assert providers["gemini_openai_gateway"].use == "vassilflow.models.patched_openai:PatchedChatOpenAI"
        assert providers["mimo"].use == "vassilflow.models.patched_mimo:PatchedChatMiMo"
        assert providers["deepseek"].use == "vassilflow.models.patched_deepseek:PatchedChatDeepSeek"
...
class TestBuildMinimalConfig:
    ...
    def test_search_tool_included(self):
        content = build_minimal_config(
            ...
            api_key_field="api_key",
            env_var="OPENAI_API_KEY",
            search_use="vassilflow.community.tavily.tools:web_search_tool",
    ...
    def test_web_fetch_tool_included(self):
        content = build_minimal_config(
            ...
            api_key_field="api_key",
            env_var="OPENAI_API_KEY",
            web_fetch_use="vassilflow.community.jina_ai.tools:web_fetch_tool",
    ...
    def test_sandbox_included(self):
        ...
        assert data["sandbox"]["use"] == "vassilflow.sandbox.local:LocalSandboxProvider"
    ...
    def test_bash_tool_disabled_by_default(self):
        ...
        assert "office_inspect" not in tool_names
    ...
    def test_can_enable_container_sandbox_and_bash(self):
        content = build_minimal_config(
            ...
            api_key_field="api_key",
            env_var="OPENAI_API_KEY",
            sandbox_use="vassilflow.community.aio_sandbox:AioSandboxProvider",
            include_bash_tool=True,
        )
        data = yaml.safe_load(content)
        assert data["sandbox"]["use"] == "vassilflow.community.aio_sandbox:AioSandboxProvider"
    ...
    def test_cli_provider_does_not_emit_fake_api_key(self):
        content = build_minimal_config(
            provider_use="vassilflow.models.openai_codex_provider:CodexChatModel",
            model_name="gpt-5.4",
    ...
    def test_patched_thinking_provider_defaults_are_preserved(self):
        ...
        data = yaml.safe_load(content)
        model = data["models"][0]
        assert model["use"] == "vassilflow.models.patched_mimo:PatchedChatMiMo"
                    ...
                    "tools": [
                        {
                            "name": "web_search",
                            "group": "web",
                            "use": "vassilflow.community.ddg_search.tools:web_search_tool",
                            "max_results": 5,
                        },
                        {
                            "name": "web_fetch",
                            "group": "web",
                            "use": "vassilflow.community.jina_ai.tools:web_fetch_tool",
                            "timeout": 10,
                        },
                        {
                            "name": "image_search",
                            "group": "web",
                            "use": "vassilflow.community.image_search.tools:image_search_tool",
                            "max_results": 5,
                        },
                        {"name": "ls", "group": "file:read", "use": "vassilflow.sandbox.tools:ls_tool"},
                        {"name": "write_file", "group": "file:write", "use": "vassilflow.sandbox.tools:write_file_tool"},
