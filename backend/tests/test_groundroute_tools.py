...
def reset_api_key_warned():
    ...
    import vassilflow.community.groundroute.tools as gr_mod
...
def mock_config_with_key():
    with patch("vassilflow.community.groundroute.tools.get_app_config") as mock:
...
def _patch_post(mock_resp: MagicMock):
    ...
    patcher = patch("vassilflow.community.groundroute.tools.httpx.Client")
...
class TestGetApiKey:
    def test_returns_config_key_when_present(self):
        with patch("vassilflow.community.groundroute.tools.get_app_config") as mock:
            ...
            from vassilflow.community.groundroute.tools import _get_api_key
...
class TestWebFetchTool:
    def test_fetch_returns_titled_content(self, mock_config_with_key):
        ...
        patcher, mock_client_cls = _patch_post(_make_search_response(payload))
        try:
            from vassilflow.community.groundroute.tools import web_fetch_tool
