...
class TestFileMemoryStorage:
    ...
    def test_get_memory_file_path_global(self, tmp_path):
        ...
        with patch("vassilflow.agents.memory.storage.get_paths", side_effect=mock_get_paths):
    ...
    @pytest.mark.parametrize(
        "invalid_name",
        ["", "../etc/passwd", "agent/name", "agent\\name", "agent name", "agent@123"],
    )
    ...
    def test_validate_agent_name_returns_canonical_identity(self):
        storage = FileMemoryStorage()

        assert storage._validate_agent_name(" Report_AGENT ") == "report-agent"
...
class TestGetMemoryStorage:
    ...
    def test_falls_back_to_file_memory_storage_on_error(self):
        ...
        with patch("vassilflow.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_class="non.existent.StorageClass")):
            storage = get_memory_storage()
            assert isinstance(storage, FileMemoryStorage)
    ...
    def test_get_memory_storage_invalid_class_fallback(self):
        ...
        with patch("vassilflow.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_class="os.path.join")):
            storage = get_memory_storage()
            assert isinstance(storage, FileMemoryStorage)
    ...
    def test_get_memory_storage_non_subclass_fallback(self):
        ...
        with patch("vassilflow.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_class="builtins.dict")):
            storage = get_memory_storage()
            assert isinstance(storage, FileMemoryStorage)
