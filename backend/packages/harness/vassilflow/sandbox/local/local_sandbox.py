...
from vassilflow.sandbox.env_policy import build_sandbox_env
from vassilflow.sandbox.local.list_dir import list_dir
...
class LocalSandbox(Sandbox):
    ...
    def _resolve_paths_in_command(self, command: str) -> str:
        ...
        def replace_match(match: re.Match) -> str:
            matched_path = match.group(0)
            # Normalize to forward slashes so bash does not interpret Windows
            # backslash sequences (\U, \a, \n, \t) as escapes.
    ...
    def _run_posix_command(
        ...
        """Run a command on POSIX without blocking on inherited background pipes."""
        timed_out = False
        ...
        stdout_capture, stdout_thread = LocalSandbox._start_pipe_drain(stdout_read_fd, "vassilflow-bash-stdout-drain")
        stderr_capture, stderr_thread = LocalSandbox._start_pipe_drain(stderr_read_fd, "vassilflow-bash-stderr-drain")
        try:
