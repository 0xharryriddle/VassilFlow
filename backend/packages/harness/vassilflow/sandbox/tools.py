...
from vassilflow.agents.thread_state import ThreadDataState
...
from vassilflow.runtime.secret_context import read_active_secrets
from vassilflow.runtime.user_context import resolve_runtime_user_id
from vassilflow.sandbox.exceptions import (
    SandboxError,
...
from vassilflow.sandbox.security import LOCAL_HOST_BASH_DISABLED_MESSAGE, is_host_bash_allowed
...
def _get_mcp_allowed_paths() -> list[str]:
    ...
    allowed_paths = []
    try:
        from vassilflow.config.extensions_config import get_extensions_config
...
def mask_secret_values(output: str, injected_env: dict[str, str] | None) -> str:
    """Redact injected secret values from bash output."""
...
CHANNEL_USER_ID_ENV = "VASSILFLOW_CHANNEL_USER_ID"
...
def _channel_identity_prefix(runtime: Runtime) -> str | None:
    """Return a POSIX shell prefix for the current IM channel user id.

    ``None`` means this is not an IM-channel run. When the key is present but
    unusable, explicitly unset the env var so shared persistent shells cannot
    inherit a stale sender id from a previous command.
    """
...
def read_current_file_content(runtime: Runtime | None, path: str) -> str:
    """Read the full current content of ``path`` using read_file resolution rules."""
    sandbox = ensure_sandbox_initialized(runtime)
    ensure_thread_directories_exist(runtime)
    if is_local_sandbox(runtime):
        thread_data = get_thread_data(runtime)
...
def write_file_tool(
    ...
    READ-BEFORE-WRITE:
    If the target file already exists, including append mode, you must read its
    current version with read_file first. Any write invalidates earlier reads,
    so re-read before consecutive modifications.
...
def str_replace_tool(
    ...
    READ-BEFORE-WRITE:
    You must read the file's current version with read_file first. Any write
    invalidates earlier reads.
