...
class RunJournal(BaseCallbackHandler):
    ...
    def on_llm_end(
        ...
        **kwargs: Any,
        ...
        for message in messages:
            ...
            if isinstance(additional_kwargs, dict) and additional_kwargs.get("vassilflow_error_fallback"):
    ...
    def _record_model_usage(
        ...
        ``cache_read_tokens`` is written sparsely so providers without cache
        reporting keep the historical bucket shape.
        """
        if total_tokens <= 0:
            return
    ...
    @staticmethod
    def _extract_cache_read(usage_dict: Mapping[str, Any]) -> int:
        """Return prompt-cache-hit input tokens from normalized usage metadata."""
