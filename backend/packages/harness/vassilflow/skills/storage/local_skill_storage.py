...
from vassilflow.config.runtime_paths import resolve_path
from vassilflow.skills.permissions import make_skill_written_path_sandbox_readable
from vassilflow.skills.storage.skill_storage import SKILL_MD_FILE, SkillStorage
from vassilflow.skills.types import SkillCategory
...
class LocalSkillStorage(SkillStorage):
    ...
    def __init__(
        ...
        if host_path is None:
            ...
            # Keep app_config as-is (may be None). Tests and config-free callers
            # use the host_path constructor; the SkillScan kill switch resolves
            # lazily at scan time when app_config is not supplied.
    ...
    async def ainstall_skill_from_archive(self, archive_path: str | Path) -> dict:
        from vassilflow.skills.installer import _scan_skill_archive_contents_or_raise
    ...
    def _prepare_skill_archive(self, path: Path, tmp_path: Path, custom_dir: Path, archive_path: str | Path) -> tuple[Path, str, Path]:
        ...
        from vassilflow.skills.installer import (
    ...
    def _commit_skill_install(self, skill_dir: Path, skill_name: str, custom_dir: Path, target: Path) -> None:
        ...
        from vassilflow.skills.installer import _move_staged_skill_into_reserved_target
