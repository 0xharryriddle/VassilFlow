"""Native deterministic scanning for VassilFlow skills.

``scan_archive_preflight()`` and ``scan_skill_dir()`` are synchronous pure
functions of their inputs; async callers must dispatch them off the event loop.
Policy is one code constant: ``CRITICAL`` blocks and every other severity is a
warning. ``enforce_static_scan()`` applies that policy and honors the
...
from vassilflow.skills.skillscan.models import (
    FindingSeverity,
    RuleSpec,
    ScanResult,
    SecurityFinding,
...
def enforce_static_scan(
    ...
    *,
    ...
    if blocked:
        ...
        raise StaticScanBlockedError(blocked, skill_name=skill_name, message=message)
...
def _nested_archive_finding(rel_path: str, prefix: bytes, read_data: Callable[[], bytes | None], scanner_errors: list[str]) -> SecurityFinding:
