...
from vassilflow.tools.builtins import ask_clarification_tool, present_file_tool, task_tool, view_image_tool
...
def _is_host_bash_tool(tool: object) -> bool:
    ...
    if group == "bash":
        return True
    if use in {"vassilflow.sandbox.tools:bash_tool", "vassilflow.sandbox.tools:bash_tool"}:
        return True
    return False
...
def get_available_tools(
    ...
    *,
    ...
    `initialize_mcp_tools()` from vassilflow.mcp module.
    ...
    if include_mcp:
        try:
            ...
            from vassilflow.mcp.cache import get_cached_mcp_tools
    ...
    try:
        from vassilflow.tools.builtins.invoke_acp_agent_tool import build_invoke_acp_agent_tool
