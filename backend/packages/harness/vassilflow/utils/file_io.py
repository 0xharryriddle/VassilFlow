...
def _default_file_io_workers() -> int:
    raw = os.getenv("VASSILFLOW_FILE_IO_WORKERS")
    if raw:
        try:
            workers = int(raw)
            if workers > 0:
            ...
            pass
        logger.warning("Invalid VASSILFLOW_FILE_IO_WORKERS value; using default file IO worker count")
    return min(32, (os.cpu_count() or 1) + 4)


_FILE_IO_EXECUTOR = ThreadPoolExecutor(max_workers=_default_file_io_workers(), thread_name_prefix="vassilflow-file-io")
...
async def run_file_io[**P, T](func: Callable[P, T], /, *args: P.args, **kwargs: P.kwargs) -> T:
    ...
    user-scoped helpers keep working inside the worker thread.
    """
