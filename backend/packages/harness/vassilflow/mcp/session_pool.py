...
def get_session_pool() -> MCPSessionPool:
    ...
    # exactly one pool and reset_session_pool() cannot null the global between
    # reading it and returning it. The critical section is tiny and never
    # awaits, so a threading.Lock is safe from async and worker-thread paths.
    with _pool_lock:
        if _pool is None:
