...
VassilFlow issue #3144: gateway request dependencies resolve
...
STARTUP_ONLY_FIELDS: dict[str, str] = {
    ...
    "log_level": (
        "apply_logging_level() runs only during app.py startup; it sets the vassilflow/app logger levels and may lower root handler thresholds so configured messages can propagate. A freshly reloaded AppConfig does not retrigger it."
    ),
