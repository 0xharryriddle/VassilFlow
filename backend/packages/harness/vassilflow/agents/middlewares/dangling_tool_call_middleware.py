...
class DanglingToolCallMiddleware(AgentMiddleware[AgentState]):
    ...
    def _sanitize_ai_message_tool_names(msg):
        """Return an AIMessage whose serialized tool-call names are non-empty."""
    ...
    def _build_patched_messages(self, messages: list) -> list | None:
        ...
        for msg in messages:
            ...
            # Inspect the original message so missing names can be classified
            # before the sanitized copy is passed to provider serialization.
            for tc in self._message_tool_calls(msg):
                tc_id = tc.get("id")
                if not tc_id:
                    continue
