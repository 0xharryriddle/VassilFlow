"""Middleware for task-level tool call progress tracking.

Structured tool result metadata drives a per-(thread, tool) state machine that
detects repeated errors, partial results, and near-duplicate successes. The
middleware can inject a model-visible hint at WARNED and hard-block a specific
tool at BLOCKED while leaving other tools available.
"""
...
def is_near_duplicate(
    ...
    """Return True if current is similar to any recent word set."""
...
def _parse_tool_meta(meta_dict: object) -> ToolResultMeta | None:
    """Safely deserialize ToolResultMeta from a raw dict."""
    ...
    try:
        ...
        logger.warning(
            "Unexpected tool meta schema, skipping progress tracking: %s",
            meta_dict,
        )
        return None
...
class ToolProgressMiddleware(AgentMiddleware[AgentState]):
    """State-machine-based tool stagnation guard."""
    ...
    def __init__(
        ...
        *,
        ...
        # threading.Lock protects both sync wrap_tool_call and async paths.
    ...
    def _update_state_from_result(
        ...
        """Update the state machine from a tool result and queue hints when needed."""
        ...
        if meta is None:
            if tool_name not in self._exempt_tools:
                logger.warning(
                    "tool_progress: vassilflow_tool_meta missing for non-exempt tool %s; verify ToolProgressMiddleware is outer of ToolErrorHandlingMiddleware",
                    tool_name,
                )
            return result
    ...
    def _reset_run_states(self, runtime: Runtime) -> None:
        """Reset all per-run tool state for the thread at the start of a run."""
        thread_id = self._thread_id(runtime)
        with self._lock:
            thread_tools = self._phase_states.get(thread_id)
            if thread_tools is None:
