...
from vassilflow.agents.thread_state import _SKILL_DESCRIPTION_MAX_CHARS, SkillEntry
...
def extract_skills(
    ...
    *,
    ...
    """Enumerate skill-file reads from AI read calls and paired tool results."""
    normalized_root = posixpath.normpath(skills_root.rstrip("/") or "/")
...
def render_skill_context(entries: list[SkillEntry]) -> str:
    ...
    for entry in entries:
        name = escape(str(entry["name"]), quote=False)
        path = escape(str(entry["path"]), quote=False)
        raw_description = entry.get("description") or ""
        if isinstance(raw_description, str):
            raw_description = " ".join(raw_description.split())[:_SKILL_DESCRIPTION_MAX_CHARS]
        description = escape(str(raw_description), quote=False)
