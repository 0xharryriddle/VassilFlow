...
``vassilflow_tool_meta`` entry in additional_kwargs. Downstream consumers
...
TOOL_META_KEY = "vassilflow_tool_meta"
...
_PARTIAL_MARKERS = (
    ...
    # Tools that return status="success" with a no-results body must still be
    # caught by stagnation detection so the model is prompted to try a different query.
    "no results found",
    "no content found",
    "no images found",
)
...
def _extract_json_error_text(content: str) -> str | None:
    """Return the error string from a JSON-wrapped error body."""
    try:
...
def stamp_exception_meta(msg: ToolMessage, exc_info: str) -> ToolMessage:
    """Stamp exception-derived metadata onto a ToolMessage."""
    attrs = _classify_error_text(exc_info)
    updated_kwargs = dict(msg.additional_kwargs or {})
    updated_kwargs[TOOL_META_KEY] = _make_meta(
        status="error",
        source="exception",
        **attrs,
    )
...
def normalize_tool_message(msg: ToolMessage) -> ToolMessage:
    """Attach structured tool metadata to a ToolMessage if not already present."""
    ...
    if msg.status == "error" and not content.startswith(_ERROR_PREFIX):
        ...
        attrs = _classify_error_text(json_error)
        meta = _make_meta(status="error", source="tool_return", **attrs)
    elif any(marker in content_lower for marker in _PARTIAL_MARKERS):
        meta = _make_meta(
