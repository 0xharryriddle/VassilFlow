...
``create_vassilflow_agent`` accepts plain Python arguments: no YAML files, no
global singletons. It sits between the raw
...
from vassilflow.agents.middlewares.tool_error_handling_middleware import ToolErrorHandlingMiddleware
...
def _assemble_from_features(
    ...
    *,
    ...
    Middleware order matches ``make_lead_agent`` (14 middlewares):
      ...
      5.   ToolResultSanitizationMiddleware (always)
    ...
    # --- [5] Remote tool-result sanitization (always) ---
    chain.append(ToolResultSanitizationMiddleware())
    ...
    if feat.vision is not False:
        ...
        if feat.sandbox is not False:
            from vassilflow.tools.builtins import view_image_tool
    ...
    if feat.subagent is not False:
        ...
        from vassilflow.tools.builtins import task_tool
