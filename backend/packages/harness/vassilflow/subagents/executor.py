...
from vassilflow.utils.messages import message_content_to_text
...
if TYPE_CHECKING:
    ...
    # `from vassilflow.subagents import SubagentExecutor`, which re-enters this
...
def _extract_final_result(final_state: Any, *, trace_id: str, name: str) -> str:
    """Extract the latest readable subagent result from a streamed state."""
...
class SubagentExecutor:
    ...
    async def _build_initial_state(self, task: str) -> tuple[dict[str, Any], list[BaseTool], "DeferredToolSetup"]:
        ...
        from vassilflow.tools.builtins.tool_search import assemble_deferred_tools, get_deferred_tools_prompt_section, get_mcp_routing_hints_prompt_section
    ...
    async def _aexecute(self, task: str, result_holder: SubagentResult | None = None) -> SubagentResult:
        ...
        try:
            ...
            logger.warning(f"[trace={self.trace_id}] Subagent {self.config.name} reached max_turns={max_turns}; recovering partial result")
