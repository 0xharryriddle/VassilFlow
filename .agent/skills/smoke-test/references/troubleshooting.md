...
This document lists common issues encountered during VassilFlow smoke testing and how to resolve them.
...
3. Or stop VassilFlow services first:
   ```bash
   make stop
   ```
...
2. Check nginx logs:
   ```bash
   cd docker && docker compose -p vassilflow-dev -f docker-compose-dev.yaml logs nginx
   ```
...
```bash
docker exec -it vassilflow-gateway sh
```

#### Clean Up All VassilFlow-Related Containers and Images
```bash
make docker-stop
cd docker && docker compose -p vassilflow-dev -f docker-compose-dev.yaml down -v
```
...
1. Check the GitHub issues for the project: https://github.com/linhlln1104/VassilFlow/issues
