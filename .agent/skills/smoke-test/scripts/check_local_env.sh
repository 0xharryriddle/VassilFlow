...
if command -v node >/dev/null 2>&1; then
    ...
    if [ "$NODE_MAJOR" -ge 22 ]; then
        ...
        echo "[FAIL] Node.js version is too old (current: $NODE_VERSION, required: 22+)"
        all_passed=false
    fi
else
...
if command -v pnpm >/dev/null 2>&1; then
    echo "[OK] pnpm is installed (version: $(pnpm --version))"
else
...
if command -v uv >/dev/null 2>&1; then
    echo "[OK] uv is installed (version: $(uv --version))"
else
...
if command -v nginx >/dev/null 2>&1; then
    echo "[OK] nginx is installed (version: $(nginx -v 2>&1))"
else
    echo "[FAIL] nginx is not installed"
...
if ! command -v lsof >/dev/null 2>&1; then
    echo "[FAIL] lsof is not installed, so port availability cannot be verified"
...
if [ "$all_passed" = true ]; then
    ...
    echo "[FAIL] Some checks failed. Please fix the issues above first"
    exit 1
fi
