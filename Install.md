...
This file is for coding agents. If the VassilFlow repository is not already open, ask the user for the correct checkout location before continuing from the repository root.

## Goal

Bootstrap a VassilFlow local development workspace on the user's machine with the least risky path available.
...
- The VassilFlow repository is cloned and the current working directory is the repo root.
- `config.yaml` exists.
...
- The user receives the exact next command to launch VassilFlow.
...
- If the current directory is not the VassilFlow repository root, ask the user for the correct checkout path, then change into the repository root.
- Confirm the current directory is the VassilFlow repository root by checking that `Makefile`, `backend/`, `frontend/`, and `config.example.yaml` exist.
...
5. Exact next command to start VassilFlow
