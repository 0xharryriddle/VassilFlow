...
[PR #3035](upstream PR #3035) addresses this boundary: when a provider stops generation with a safety signal while the response still contains tool calls, VassilFlow should suppress those tool calls first and record the turn as a safety termination event.
