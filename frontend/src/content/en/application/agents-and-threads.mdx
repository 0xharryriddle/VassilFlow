...
description: VassilFlow App supports multiple named agents and maintains conversation state across sessions through threads and checkpointing.
---
...
VassilFlow App supports multiple named agents and maintains conversation state across sessions through threads and checkpointing.
...
<Callout type="info">
  ...
  <code>VassilFlowClient</code> in direct Python integrations.
</Callout>
...
```
backend/.vassilflow/threads/{thread_id}/user-data/
```

This directory is mounted into the sandbox as `/mnt/user-data/` for the agent to read and write.

In Docker deployments, this path is bind-mounted from the host so that thread data persists across container restarts. Set `VASSILFLOW_ROOT` to the absolute path of your VassilFlow repository root to ensure the correct host path is used.
