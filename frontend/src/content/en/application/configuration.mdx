---
title: Configuration
description: VassilFlow App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
---
...
VassilFlow App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
...
Install Ollama provider: `cd backend && uv add 'vassilflow-harness[ollama]'`
...
Install: `cd backend && uv add 'vassilflow-harness[aio-sandbox]'`
...
tools:
  ...
  - use: vassilflow.community.ddg_search.tools:web_search_tool # default, no key required
...
By default, VassilFlow uses an SQLite checkpointer for thread state persistence:

```yaml
checkpointer:
  type: sqlite
  connection_string: checkpoints.db # stored in backend/.vassilflow/
```
...
checkpointer:
  type: postgres
  connection_string: postgresql://user:password@localhost:5432/vassilflow
```
