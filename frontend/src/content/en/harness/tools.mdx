...
<Callout type="info" emoji="🔧">
  Tools are the actions the Lead Agent can take. VassilFlow provides built-in
...
VassilFlow organizes tools into four categories:
...
tools:
  ...
  - use: vassilflow.sandbox.tools:str_replace_tool
  - use: vassilflow.sandbox.tools:bash_tool # requires host bash or container sandbox
```
...
Install: `cd backend && uv add 'vassilflow-harness[tavily]'`
...
Install: `cd backend && uv add 'vassilflow-harness[exa]'`
...
Install: `cd backend && uv add 'vassilflow-harness[firecrawl]'`
...
Install: `cd backend && uv add 'vassilflow-harness[groundroute]'`
