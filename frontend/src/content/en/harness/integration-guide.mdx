...
description: VassilFlow Harness is not only a standalone application. It is a Python library you can import and use inside your own backend, API server, automation system, or multi-agent orchestrator.
---
...
<Callout type="info" emoji="🔌">
  VassilFlow Harness can be embedded into any Python application. This guide
  covers the integration patterns for using VassilFlow as a library inside your
  own system.
</Callout>

VassilFlow Harness is not only a standalone application. It is a Python library you can import and use inside your own backend, API server, automation system, or multi-agent orchestrator.

## Embedding VassilFlowClient

The primary integration point is `VassilFlowClient`. It wraps the LangGraph runtime and exposes a clean API for sending messages and streaming responses from any Python application.

```python
from vassilflow.client import VassilFlowClient
from vassilflow.config import load_config

# Load configuration (reads config.yaml or VASSILFLOW_CONFIG_PATH)
load_config()
...
VassilFlow Gateway is itself a FastAPI application. You can mount it as a sub-application or router:
...
# Mount the VassilFlow gateway router.
# Source-app module path when running from the VassilFlow repository.
from app.gateway.app import app as gateway_app
app.mount("/vassilflow", gateway_app)
```

Or use `VassilFlowClient` directly in your own FastAPI routes with streaming:
...
VassilFlow Harness is built on LangGraph. The Lead Agent is a standard LangGraph graph. You can compose it with your own LangGraph nodes and graphs:
...
```python
import os
os.environ["VASSILFLOW_CONFIG_PATH"] = "/path/to/my-vassilflow-config.yaml"
...
```python
from vassilflow.config import load_config
load_config(config_path="/path/to/my-vassilflow-config.yaml")
```

## MCP server integration

VassilFlow can expose its agent as an MCP server, allowing other MCP-compatible systems to call it as a tool. Refer to the VassilFlow repository for MCP server integration examples.
