---
title: Memory
description: Memory is a runtime feature of the VassilFlow Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
---
...
<Callout type="info" emoji="💾">
  Memory lets VassilFlow carry useful information across sessions. The agent
...
Memory is a runtime feature of the VassilFlow Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
...
memory:
  ...
  # Default: {base_dir}/memory.json  (resolves to backend/.vassilflow/memory.json)
...
VassilFlow supports two levels of memory:
...
- Base directory: `backend/.vassilflow/`
- Global memory: `backend/.vassilflow/memory.json`
- Per-agent memory: `backend/.vassilflow/agents/{agent_name}/memory.json`
