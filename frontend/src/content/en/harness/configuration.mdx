---
title: Configuration
description: "VassilFlow's configuration system is designed around one goal: every meaningful behavior should be expressible in a config file, not hardcoded in the application. This makes deployments reproducible, auditable, and easy to customize per environment."
---
...
<Callout type="info" emoji="⚙️">
  All VassilFlow Harness behaviors are driven by <code>config.yaml</code>. One
  file controls which models are available, how the sandbox runs, what tools are
  loaded, and how each subsystem behaves.
</Callout>

VassilFlow's configuration system is designed around one goal: every meaningful behavior should be expressible in a config file, not hardcoded in the application. This makes deployments reproducible, auditable, and easy to customize per environment.

## Config file location

VassilFlow resolves `config.yaml` using the following priority order:

1. The path passed to `AppConfig.from_file(config_path)` explicitly.
2. The `VASSILFLOW_CONFIG_PATH` environment variable, falling back to legacy `DEER_FLOW_CONFIG_PATH`.
...
```bash
export VASSILFLOW_CONFIG_PATH=/path/to/my-config.yaml
```

During the transition from VassilFlow to VassilFlow, prefer `VASSILFLOW_*`
environment variable names. Legacy `DEER_FLOW_*` names still work as fallbacks,
and `VASSILFLOW_*` wins when both are set.
...
This pattern is how VassilFlow achieves pluggability without hardcoding class references.
