...
2. `VassilFlowSummarizationMiddleware` — context compression (if enabled)
...
The `VassilFlowSummarizationMiddleware` wraps LangChain's base summarization behavior and is one of the most impactful middlewares for long-horizon tasks. Here is the full configuration reference:
