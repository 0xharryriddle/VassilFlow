---
title: 部署指南
description: 本指南涵盖 VassilFlow 应用所有支持的部署方式：本地开发、Docker Compose 以及使用 Kubernetes 管理沙箱的生产环境。
---
...
本指南涵盖 VassilFlow 应用所有支持的部署方式：本地开发、Docker Compose 以及使用 Kubernetes 管理沙箱的生产环境。

## 本地开发部署

本地工作流是运行 VassilFlow 最快的方式，所有服务作为原生进程在你的机器上运行。
...
线程数据存储在 `backend/.vassilflow/threads/`。在 Docker 部署中，此目录会绑定挂载到 gateway 容器中。

为避免容器重建时数据丢失：

1. 将 `VASSILFLOW_ROOT` 设置为仓库根目录的绝对路径。
...
nginx 将流量路由到前端或 Gateway。`/api/langgraph/*` 会被重写到 Gateway 的 LangGraph-compatible `/api/*` 路由，因此不需要单独的 LangGraph 服务。

### 认证配置

VassilFlow 应用使用 [Better Auth](https://www.better-auth.com/) 进行会话管理。在生产环境中：
