...
<Callout type="info" emoji="🔧">
  工具是 Lead Agent 可以采取的行动。VassilFlow 提供内置工具、社区集成、MCP
...
安装：`cd backend && uv add 'vassilflow-harness[tavily]'`
...
安装：`cd backend && uv add 'vassilflow-harness[exa]'`
...
安装：`cd backend && uv add 'vassilflow-harness[groundroute]'`
