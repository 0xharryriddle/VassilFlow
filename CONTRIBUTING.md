# Contributing to VassilFlow

Thank you for your interest in contributing to VassilFlow! This guide will help you set up your development environment and understand our development workflow.
...
|---------|-----------|------------|-------|
| `make dev` on one machine | 4 vCPU, 8 GB RAM | 8 vCPU, 16 GB RAM | Best when VassilFlow uses hosted model APIs. |
...
`2 vCPU / 4 GB` environments often fail to start reliably or become unresponsive under normal VassilFlow workloads.
...
```text
unable to get image 'vassilflow-gateway': permission denied while trying to connect to the Docker daemon socket at unix:///var/run/docker.sock
```
...
4. Verify Docker access:
   ...
   docker ps
   ```
5. Retry the VassilFlow command:
   ```bash
   make docker-stop
   make docker-start
   ```
...
VassilFlow is an AI project and we welcome AI-assisted contributions. To help
...
By contributing to VassilFlow, you agree that your contributions will be licensed under the [MIT License](./LICENSE).
